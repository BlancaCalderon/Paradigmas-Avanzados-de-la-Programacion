...
__global__ void kernelEncontrarCaminos(int* dev_tablero, int** dev_caminos, int numFila, int numCol, int *dev_index, int cX, int cY)
{
    ...
    {
        printf("\nHilo numero %d coincide posicion auxiliar inicial \n", pos, posAux);

        while ((posAux < numCol * numFila) && !encontrado && !camino_invalido)
        {
            sigfila = posAux / numCol;             //Fila en la que se encuentra el siguiente elemento
            sigcol = posAux - sigfila * numCol;   //Columna en la que se encuentra el siguiente elemento
            
            //filaActual = sigfila - 1;
            colActual = sigcol - 1;

            printf("\n FILA y COLUMNA del siguiente elemento %d %d hilo %d\n", sigfila, sigcol, pos);
            printf("\n SIGUIENTE FILA misma columna %d %d hilo %d\n", sigfila + 1, colActual, pos);

            printf("\n Condicion siguiente columna: hilo %d == %d , continuamos misma fila: Siguiente %d - PosAux %d \n", pos, posAux, sigfila, sigcol);
            printf("\n Avanzamos 1 fila a su misma columna hilo %d hilo == %d\n", posAux, posAux - 1 + numCol);

            if (dev_tablero[pos] == dev_tablero[posAux] && sigcol > 0)                                  //Nos desplazamos a la derecha
            {
                printf("\nContinua por posicion (fila) hilo %d", pos);
            ...
            else if (dev_tablero[pos] == dev_tablero[posAux - 1 + numCol] && (sigfila + 1 < numFila))  //Nos desplazamos hacia abajo
            {
                printf("\nAvanza 1 fila a su misma columna %d hilo ", pos);
                    dev_camino[index] = posAux;
                    posAux = posAux - 1 + numCol;
                    index += 1;
