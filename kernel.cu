...
__global__ void kernelGenerarTablero(int* dev_tablero, int dev_semilla, int dificultad)
{
 ...
 __global__ void kernelEncontrarCaminos(int* dev_tablero, int* dev_camino, int numFila, int numCol, int* dev_index, int pos_encontrar, bool* dev_encontrado)
    {
        ...
        if (dev_tablero[pos] == dev_tablero[pos_encontrar] && pos_encontrar == pos)
        {
            ...
            while ((posAux < numCol * numFila) && !encontrado && !camino_invalido)
            {
                ...
                else if (dev_tablero[pos] == dev_tablero[posAux + numCol] && (posAux + numCol) != ultima_posicion && (posAux + numCol) < numCol * numFila)  //Hacia abajo
                {
                    ultima_posicion = posAux;
                ...
                else if (dev_tablero[pos] == dev_tablero[posAux - 1] && col_anterior >= 0 && (posAux - 1) != ultima_posicion)           //Izquierda
                {
                    if (posAux != pos_encontrar) {
                        dev_camino[dev_index[0]] = posAux;
                        atomicAdd(&dev_index[0], 1);
                ...
                else if (dev_tablero[pos] == dev_tablero[posAux - numCol] && (posAux - numCol) != ultima_posicion && (posAux - numCol) >= 0 && filaActual >= 0 && filaActual <= numFila)  //ARRIBA
                {
...
void encontrarCamino(int* h_tablero, int numFilas, int numColumnas, int coordX, int coordY)
{
    ...
    dim3 threadsInBlock(size);
    while (h_encontrado) {
        kernelEncontrarCaminos << <1, threadsInBlock >> > (dev_Tablero, dev_camino, numFilas, numColumnas, dev_index, pos_encontrar, dev_encontrado);
    ...
    /*
    h_encontrado = true;
    for (int i = 0; i < h_index[0]; i++) { 
        while (h_encontrado) {
            kernelEncontrarCaminos << <1, threadsInBlock >> > (dev_Tablero, dev_camino, numFilas, numColumnas, dev_index, h_caminos[i], dev_encontrado);
