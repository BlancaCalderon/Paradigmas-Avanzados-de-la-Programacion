...
object Main {
  ...
  def programaTeclado(vidas:Int): Unit = {
    ...
    mostrarTablero(tablero, 0, numFilas, numCol)
    jugar(numFilas, numCol, limiteNum, tablero, modoJuego, vidas)   //Llamamos a la funcion jugar, que se encargará de comenzar la partida
  }
  ...
  def seleccionModoJuego(modoJuego: Char, numFilas: Int, numCol: Int, dificultad: Int, tablero: List[Int]): Int = {
    ...
    else if (modoJuego == 'm' || modoJuego == 'M') {    //Modo manual, usuario introduce las coordenadas
  ...
  def jugar(numFilas: Int, numCol: Int, dificultad: Int, tablero: List[Int], modoJuego: Char, vidas: Int): Unit = {
    ...
    vidas match {
      case 0 =>
      {
        ...
        if(otraVez == 'y')
        {
          programaTeclado(5)    //Vuelve a pedir valores de la partida (numero filas, columnas, dificultad y modojuego)
        }
        else  println("Adios!")
      }
      ...
      case _ => {
        ...
        //Obtiene el color de la casilla con la que vamos a realizar la jugada
  ...
  def reemplazarPosiciones(pos:Int, tablero:List[Int], N:Int, M:Int, dificultad:Int): List[Int] = {
    ...
    contador match
    {
      ...
      {
        val nuevo:List[Int] = reemplazarAux(0, tablero, N, M, dificultad, contador, N*M)  //Llamada metodo auxiliar
        //Si aún no se han reemplazado todas sus posiciones devovelvera un numero mayor que 0
        if (contarPosicionesBorradas(nuevo) > 0) reemplazarPosiciones(0, nuevo, N, M, dificultad) //Llamada recursiva
        else nuevo
      }
    }
  }
      ...
      case _ =>           //Caso recursivo
      {
        if (tablero(pos) == -1)   //La posicion está eliminada
        {
          ...
          if (contiene(tablero, pos - M, N * M) && pos - M >= 0 && filaActual > 0 && filaActual <= N && tablero(pos - M) != -1) //Si la posicion de arriba es distinta de -1 se la asignamos a la posicion que nos llega y se la quitamos a la de arriba
          {
            ...
            insertarElementoPosicion(color, pos, reemplazarAux(pos + 1, nuevo, N, M, dificultad, contador, size - 1)) //Recursividad
          }
   ...
   * @param n
   * @param l
   * @return lista con los N primeros elementos de l
   */
